...
namespace Parser {
  ...
  eResult SyntaxAnalyzer::TypeDef(void) {
    ...
    char* typeName = new char[pi_id.cstr_len_ + 1];
    memset(typeName, 0, pi_id.cstr_len_ + 1);
    strncpy(typeName, pi_id.data_.cstr_, pi_id.cstr_len_);

    // create TypeNode
    AST::UserType* ut = AST::UserType::Get(ac_, pi_type.data_.type_, typeName);
    AST::TypeNode* user_type = new AST::TypeNode(ut);

    AST::TypedefNode* typedef_node = new AST::TypedefNode(user_type, typeName);
    delete[] typeName;
