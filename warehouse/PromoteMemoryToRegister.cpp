...
struct AllocaInfo {
  ...
  /// 지정된 alloca의 사용을 스캔하고, 이 alloca의 사용에 대해 추론하기 위해 패스의 나머지 부분에서 사용하는 AllocaInfo를 채웁니다.
  void AnalyzeAlloca(AllocaInst *AI) {
    ...
    // alloca 명령의 사용을 스캔하면서 저장소를 추적하고, alloca에 대한 모든 로드와 저장이 같은 기본 블록 내에 있는지 결정합니다.
    for (User *U : AI->users()) {
      Instruction *User = cast<Instruction>(U);
 
      // alloca 의 User 는 Load or Store 둘중 하나이다.
      if (StoreInst *SI = dyn_cast<StoreInst>(User)) {
        // Remember the basic blocks which define new values for the alloca
        // alloca의 defs BB 를 저장
        ...
        // reads.
        // 그렇지 않으면 로드 명령이어야 하며 변수 읽기를 추적해야 합니다.
        UsingBlocks.push_back(LI->getParent());
      }
 
      // 이 alloca 가 여러 BB 에서 사용되는지 정보 저장.
...
static void convertMetadataToAssumes(
    LoadInst *LI, 
    Value *Val,
    const DataLayout &DL, 
    AssumptionCache *AC,
  ...
  // load 가 null이 아닌 것으로 표시된 경우 이 로드를 지울 때 해당 정보가 손실되는 것을 원하지 않습니다. 그래서 우리는 그것을 가정으로 보존합니다. 위반이 즉각적인 정의되지 않은 동작이라고 가정하는 동안 !nonnull은 poison을 반환하므로 값이 non-poison으로 알려진 경우에만 이를 수행할 수 있습니다.
...
/// store 명령이 1개뿐일 때 가능한 한 많은 load들을 다시 작성.
///
...
/// store 가 1개만 있는 경우 domtree를 사용하여 지배적인 모든 load를 저장된 값으로 간단하게 대체할 수 있습니다. 이렇게 하고 alloca를 성공적으로 승격시킨 경우 true를 반환합니다. 이것이 false를 반환하면 단일 store 에 의해 지배되지 않는 일부 load 가 있으므로 undef로 phi-ed되어야 합니다. 이 경우 표준 alloca 승격 알고리즘으로 돌아갑니다.
static bool rewriteSingleStoreAlloca(
  ...
  // StoringGlobalVal : Instruction 이면 false, 아니면 true (전역 변수인 경우)
  ...
  for (User *U : make_early_inc_range(AI->users())) {
    ...
    // 좋아, 만약 우리가 alloca로부터 load 를 가지고 있다면, 우리는 그것을 alloca에 저장된 유일한 값으로 대체하기를 원한다. 값이 store 에 의해 지배된다면 우리는 이것을 할 수 있습니다. 그렇지 않은 경우 나머지 mem2reg 기계를 사용하여 필요에 따라 phi 노드를 삽입합니다.
    if (!StoringGlobalVal) { // Non-instructions are always dominated.
      if (LI->getParent() == StoreBB) {
        // If we have a use that is in the same block as the store, compare the
        // indices of the two instructions to see which one came first.  If the
        // load came before the store, we can't handle it.
        // store 와 같은 블록에 있는 사용이 있는 경우, 두 명령어의 인덱스를 비교하여 어느 것이 먼저인지 확인합니다. 만약 load 가 store 보다 먼저 온다면, 우리는 그것을 처리할 수 없습니다.
        if (StoreIndex == -1)
        ...
        // out.
        // load 와 store가 다른 블록에 있는 경우 BB 도미넌스를 사용하여 관계를 확인합니다. store 가 사용을 금지하지 않으면 구제 금융을 받으십시오.
        Info.UsingBlocks.push_back(LI->getParent());
        continue;
      }
    }
 
    // 단일 Store 의 src 가 Instruction 일 경우.
    ...
    /*
    아래와 같은 예제 코드가 있다고 하자.
    아래의 코드를 통해서 alloca 에 저장하는 store 와 
    그 alloca 를 다시 불러오는 load 를 뛰어 넘어서 바로 가상 레지스터에
    접근 하는 코드로 변경한다.
    
    %a.alloca alloca i32
    ...
    %3 = add %1, %2
    store %3, *i32 %a.alloca
    %4 = load i32, %a.alloca
    ...
    %6 = mul %4, %5 <= 여기서 store, load 를 뛰어 넘어서 바로 %3 으로 치환한다.
  ...
  // 삭제할 store 에 대한 할당 추적 정보를 업데이트합니다.
...
/// using the Alloca.
/// 많은 alloca는 단일 기본 블록 내에서만 사용됩니다. 이 경우 Alloca를 사용하여 기본 블록에 대해 단일 선형 패스를 수행하여 CFG를 순회하고 잠재적으로 쓸모 없는 많은 PHI 노드를 삽입하지 마십시오.
///
...
void PromoteMem2Reg::run() {
  ...
  for (unsigned AllocaNum = 0; AllocaNum != Allocas.size(); ++AllocaNum) {
    ...
    // 1. promote 불 가능한 alloca 를 조사
    assert(isAllocaPromotable(AI) && "Cannot promote non-promotable alloca!");
    assert(AI->getParent()->getParent() == &F &&
           "All allocas should be in the same function, which is same as DF!");
 
    removeIntrinsicUsers(AI); // 2. debug 정보 같은 intrinsic user 들을 삭제
    ...
    // 각 할당에 대한 읽기 및 쓰기 위치 집합을 계산합니다. 이는 각 변수의 '사용' 및 '정의'를 찾는 것과 유사합니다.
    Info.AnalyzeAlloca(AI);
 
    // If there is only a single store to this value, replace any loads of
    // it that are directly dominated by the definition with the value stored.
    // 이 값에 대한 단일 store만 있는 경우, 정의에 의해 직접 지배되는 모든 load들을 저장된 값으로 바꿉니다.
